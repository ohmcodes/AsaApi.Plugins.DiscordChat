...
void RepairItemsCallback(AShooterPlayerController* pc, FString* param, int, int)
{
	...
	if (command.is_null() || (!command.is_null() && command.value("Enabled", false) == false))
	{
		...
		AsaApi::GetApiUtils().SendNotification(pc, FColorList::Red, 1.3f, 15.0f, nullptr, DiscordGlobalChat::config["Messages"].value("PermErrorMSG", "You don't have permission to use this command.").c_str());
	...
	if (Points(pc->GetEOSId(), command.value("Cost", 0), true) == false)
	{
		...
		AsaApi::GetApiUtils().SendNotification(pc, FColorList::Red, 1.3f, 15.0f, nullptr, DiscordGlobalChat::config["Messages"].value("PointsErrorMSG", "Not enough points.").c_str());
	...
	bool ignoreMaterials = DiscordGlobalChat::config["General"]["IgnoreInvRepairRequirements"].get<bool>();
	...
	{
		AsaApi::GetApiUtils().SendNotification(pc, FColorList::Green, 1.3f, 15.0f, nullptr, DiscordGlobalChat::config["Messages"].value("RepairItemsMSG", "All items has been repaired. {}").c_str(), pc->GetCharacterName().ToString());
